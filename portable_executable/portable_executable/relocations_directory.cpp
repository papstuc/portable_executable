...
void portable_executable::relocations_iterator_t::load_block(const raw_relocation_block_descriptor_t* raw_relocation_block_descriptor)
{
    ...
    {
        const std::uint64_t real_block_size = this->current_raw_relocation_block_descriptor->size_of_block - sizeof(raw_relocation_block_descriptor_t);

        this->current_relocation_block.max_entry_index = static_cast<std::uint32_t>(real_block_size / sizeof(relocation_entry_descriptor_t));
...
portable_executable::relocations_iterator_t& portable_executable::relocations_iterator_t::operator++()
{
    ...
    {
        this->load_block(reinterpret_cast<const raw_relocation_block_descriptor_t*>(reinterpret_cast<std::uint64_t>(this->current_raw_relocation_block_descriptor) + this->current_raw_relocation_block_descriptor->size_of_block));
    }
    else
    {
